...
public:
    ...
    void iremove(int value){
        Node *node = head; // Cabeça pra começar a lista
        Node *anterior = nullptr; //auxiliar

        while(node != nullptr && node->value == value)//percorrer a lista
                anterior = node; //
                node = node->next;

         if(node == nullptr) //verificando se a lista é válida,vazia.
             return;
         if(anterior == nullptr)//remover do inicio
             head = node->next;
